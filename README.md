...
- **Hook enable/disable at runtime** — toggle individual tweaks on/off from the overlay without restarting; requires HookEngine runtime-safety work.
- **Hot-reload .ini** — detect external edits and reload without restart.
- **CustomCVars overlay** — simple text editor for user-defined CVar entries.
- **Telemetry overlay** — last hook resolve time, detour invocation counters.
